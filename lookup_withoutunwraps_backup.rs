...
impl Lookup {
    ...
    pub fn road_slk_from_coordinate(
        ...
        let (index, distance) = self
            ...
            .ok_or(PyException::new_err("No features found"))?;

        let feature = &self.features[index];

        let distance_along_object = 
            feature
            .geometry.0
            .line_locate_point(&pnt)
            .ok_or(PyException::new_err("Projection failed. LineString.line_locate_point() from `geo` library did not produce a result"))?;
    ...
    pub fn coordinate_from_road_slk(
        ...
        let list_of_lists:Vec<PyObject> = self
            .index
            .get(road)
            .ok_or(PyException::new_err("Road number was not found in dataset"))?
    ...
    pub fn from_binary(input: &PyBytes) -> PyResult<Self> {
        let lookup:Self = 
            bincode::deserialize(input.as_bytes())
            .map_err(
                |_| PyException::new_err(
                    "`bincode` library failed to decode the provided bytes object. Not possible to provide a reason. Did you encode the bytes using the same version of this library?"
                )
            )?;
        Ok(lookup)
    }
    ...
    pub fn from_dict(python_dictionary: &PyDict) -> PyResult<Self> {
        let mut features: Vec<ExtractedFeature> = python_dictionary
            .get_item("features")
            .ok_or(PyException::new_err("`features` key not found"))?
            .extract::<Vec<&PyAny>>()?
            .into_iter()
            .map(
                |feature| feature
                .extract::<ExtractedFeature>()
                .map_err(|_| PyException::new_err("One of the features could not be extracted. I don't know how to recover this deserialize process. Aborting."))
                .unwrap()
            )
            .collect();
            ...
            if b_feature_is_new_road || b_feature_is_new_cwy {
                ...
                match result.entry(b_feature.properties.road.clone()) {
                    ...
                    Entry::Occupied(mut e) => {
                        e.insert(
                            e.get().with_updated_cwy(&a_feature.properties.cwy, (current_slice_start, i)),
                        );
                    }
                }
                current_slice_start = i;
